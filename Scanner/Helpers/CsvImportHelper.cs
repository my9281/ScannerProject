...
namespace Scanner.Helpers
{
    public sealed class CsvImportHelper
    {
        ...
        public IReadOnlyList<WorkOrderRemark> ImportUrgentWorkOrders(string filePath)
        {
            ...
            using (var parser = new TextFieldParser(filePath, Encoding.UTF8))
            {
                ...
                while (!parser.EndOfData)
                {
                    ...
                    result.Add(new WorkOrderRemark { Id = $"CSV-{rowNumber}-{Guid.NewGuid():N}", Sn = NullIfEmpty(sn), TrackingNumber = NullIfEmpty(tracking), Remark = GetField(fields, RemarkColumn), IsUrgent = true, RemarkTimestamp = DateTimeOffset.UtcNow.ToUnixTimeSeconds() });
                }
            }
            return result;
        }
        ...
        private static bool IsHeader(string[] fields)
        {
            ...
            return sn.IndexOf("SN", StringComparison.OrdinalIgnoreCase) >= 0 || sn.Contains("序列号") || sn.Contains("编码") || tracking.Contains("运单") || tracking.IndexOf("Tracking", StringComparison.OrdinalIgnoreCase) >= 0 || remark.Contains("备注");
        }
