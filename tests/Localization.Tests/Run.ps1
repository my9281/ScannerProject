...
& "$msbuildPath/Roslyn/csc.exe" /nologo /target:exe "/out:$repoPath/Scanner/bin/Release/VerifyLocalization.exe" "/reference:$repoPath/Scanner/bin/Release/Scanner.exe" $references "$PSScriptRoot/VerifyLocalization.cs"
if ($LASTEXITCODE -ne 0) { throw 'Localization test compilation failed' }
& "$repoPath/Scanner/bin/Release/VerifyLocalization.exe" $repoPath "$PSScriptRoot/bin/previews"
