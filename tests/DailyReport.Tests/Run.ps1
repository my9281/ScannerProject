...
& "$msbuildPath/Roslyn/csc.exe" /nologo /target:exe "/out:$repoPath/Scanner.WPF/bin/Release/VerifyDailyReport.exe" "/reference:$repoPath/Scanner.WPF/bin/Release/Scanner.WPF.exe" $references "$PSScriptRoot/Program.cs"
if ($LASTEXITCODE -ne 0) { throw 'Daily report test compilation failed' }
& "$repoPath/Scanner.WPF/bin/Release/VerifyDailyReport.exe"
