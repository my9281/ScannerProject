...
& "$msbuildPath/Roslyn/csc.exe" /nologo /target:exe "/out:$repoPath/Scanner/bin/Release/VerifyChecklistSimplifiedExport.exe" "/reference:$repoPath/Scanner/bin/Release/Scanner.exe" $references "$PSScriptRoot/Program.cs"
if ($LASTEXITCODE -ne 0) { throw 'Simplified export test compilation failed' }
& "$repoPath/Scanner/bin/Release/VerifyChecklistSimplifiedExport.exe"
