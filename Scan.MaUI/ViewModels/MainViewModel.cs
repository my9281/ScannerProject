...
public sealed class MainViewModel : ViewModelBase
{
    ...
    {
        string code=ScanCode.Trim(); if(string.IsNullOrWhiteSpace(code)){SetStatus(L.Get("EmptyCode"),true);RequestFocus();return;}
        if(ScanService.IsGs1AreaCode(code)){await _speech.SpeakGs1AreaWarningAsync();SetStatus(L.Get("Gs1AreaWarning"),true);ScanCode=string.Empty;RequestFocus();return;}
        Busy=true;
        try
        {
            ScanResult result=await _scan.RecordAsync(code); WorkOrderMatch? match=_workOrders.Resolve(result.Code, result.Oid.IsOid); WorkOrderRemark? workOrder=match?.WorkOrder; string printCode=match?.GetPrintCode(result.Code) ?? result.Code; string model=_models.Find(printCode);
            ...
            else if(copies>0){if(string.IsNullOrWhiteSpace(model)&&ModelSelectionRequested is not null) model=await ModelSelectionRequested(_models.GetModels())??string.Empty;if(string.IsNullOrWhiteSpace(model)){SetStatus($"未选择型号：{printCode}",true);return;}await _printer.PrintAsync(printCode,copies,workOrder,model,PaperSize);await _speech.SpeakTailAsync(printCode);SetStatus(L.Format("MobilePrinted", copies, printCode),false);}
            else {if(!result.Oid.IsOid) await _speech.SpeakTailAsync(result.Code);SetStatus(result.WasRecorded?L.Format("MobileRecorded", result.Code):L.Format("MobileDuplicate", result.Code),false);}
