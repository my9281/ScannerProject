...
public sealed class OperationsPageController : ControllerBase
{
    ...
    private readonly Scan.MaUI.Windows.IWindow _window;
    ...
    public OperationsPageController(IOperationsPageView view, Scan.MaUI.Windows.IWindow window, ChecklistDataCache baseData) { _view = view; _window = window;  _baseData = baseData;  RefreshBase();  }
    private void RefreshBase()
    {
        ...
        _view.OutboundStatusLabel.Text = "请选择 SKU / SN 文本，使用全局基础表匹配";
    ...
    {
        await RunAsync(async () => { var path = await PickAndCacheAsync("导入全局基础 Excel"); if (path is null) return; _baseData.ImportBase(path); RefreshBase(); });
    }
    public async void PickInboundSn_Clicked(object? sender, EventArgs e)
    {
        await RunAsync(async () => { _inboundSn = await PickAndCacheAsync("选择待匹配 SN 文本"); if (_inboundSn is null) return; _serialNumbers = File.ReadAllLines(_inboundSn).Select(x => x.Trim().TrimStart('\uFEFF')).Where(x => x.Length > 0 && !x.Equals("SN", StringComparison.OrdinalIgnoreCase)).ToList(); _view.InboundSnLabel.Text = _inboundSn; RefreshInboundStatus(); });
    }
    public async void ExportInboundMatch_Clicked(object? sender, EventArgs e)
    {
        await RunAsync(async () =>
        {
            ...
            string path = OutputPath("InboundDetectionResult", ".txt"); await File.WriteAllLinesAsync(path, lines, new UTF8Encoding(true)); _view.InboundStatusLabel.Text = $"匹配 {matched}，未匹配 {_serialNumbers.Count - matched}"; await ShareOutputAsync(path);
        });
    }
    public async void ExportCurrentMonth_Clicked(object? sender, EventArgs e)
    {
        await RunAsync(async () =>
        {
            ...
            string path = OutputPath("CurrentMonthPending_" + month.ToString("yyyyMM"), ".csv"); await File.WriteAllLinesAsync(path, lines, new UTF8Encoding(true)); _view.InboundStatusLabel.Text = $"已生成 {rows.Count} 条待检测记录"; await ShareOutputAsync(path);
        });
    }
    public async void PickOutboundText_Clicked(object? sender, EventArgs e) { await RunAsync(async () => { _outboundText = await PickAndCacheAsync("选择 SKU / SN 文本"); _view.OutboundTextLabel.Text = _outboundText ?? "尚未选择"; LoadOutbound(); }); }
    private void LoadOutbound()
    {
        _outboundRecords = new List<OutboundInspectionRecord>();
        _view.OutboundStatusLabel.Text = "请导入全局基础表并选择 SKU / SN 文本。";
        if (_outboundText is null || _baseData.Records.Count == 0) return; _outboundRecords = OutboundInspectionService.Build(_outboundText, _baseData.Records); int matched = _outboundRecords.Count(x => x.IsMatched); int skus = _outboundRecords.Where(x => !string.IsNullOrWhiteSpace(x.Sku)).Select(x => x.Sku).Distinct(StringComparer.OrdinalIgnoreCase).Count(); _view.OutboundStatusLabel.Text = $"共 {_outboundRecords.Count} 条，{skus} 个 SKU，匹配 {matched}，未匹配 {_outboundRecords.Count - matched}";
    }
    public async void ExportOutbound_Clicked(object? sender, EventArgs e) { await RunAsync(async () => { if (_outboundRecords.Count == 0) throw new InvalidOperationException("请先选择两个来源文件。"); string path = OutputPath("出库检测结果", ".xlsx"); OutboundInspectionXlsxWriter.Write(path, _outboundRecords); await ShareOutputAsync(path); }); }
    public async void PickFeeTemplate_Clicked(object? sender, EventArgs e) { await RunAsync(async () => { _feeTemplate = await PickAndCacheAsync("选择库位费模板 Excel"); _view.FeeTemplateLabel.Text = _feeTemplate ?? "尚未选择"; }); }
    public async void ExportFee_Clicked(object? sender, EventArgs e)
    {
        await RunAsync(async () => { if (_feeTemplate is null || _baseData.Records.Count == 0) throw new InvalidOperationException("请先选择模板和基础 Excel。"); string path = OutputPath("库位付费比对结果", ".xlsx"); var summary = LocationFeeComparisonService.Build(_feeTemplate, _baseData.Records, path); _view.FeeStatusLabel.Text = $"{summary.SheetCount} 个工作表，{summary.RowCount} 行，匹配 {summary.MatchedCount}，未匹配 {summary.UnmatchedCount}"; await ShareOutputAsync(path); });
    }

    private void RefreshInboundStatus() => _view.InboundStatusLabel.Text = $"基础数据 {_inboundRecords.Count} 条；SN {_serialNumbers.Count} 条";
    ...
    private async Task RunAsync(Func<Task> action) { try { _view.IsBusy = true; await action(); } catch (Exception ex) { await _window.ShowAlertAsync("处理失败", ex.Message, "确定"); } finally { _view.IsBusy = false; } }
