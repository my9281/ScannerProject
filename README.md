...
Windows 扫描时还会在 TXT 同目录维护一个同名 XLSX，列为“型号、SN、OID”。扫描 OID 时先写入 C 列，下一条 SN 会补入同一行的 A、B 列；连续扫描 SN 时按行顺序追加。“打开扫描记录”和“打开 Excel”分别打开当前 TXT 与 XLSX。点击“新文件”会创建一组带时间戳的 TXT/XLSX，并把当前 TXT 文件名保存到用户 `config`；后续打开、扫码记录及上传都改用这组文件。
